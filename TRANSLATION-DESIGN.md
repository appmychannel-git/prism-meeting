...
## 6. 확정된 결정 사항 (2026-07-28)

- **지원 언어(11종)**: 한국어 `ko`, 영어 `en`, 카자흐어 `kk`, 러시아어 `ru`,
  키냐르완다 `rw`, 프랑스어 `fr`, 일본어 `ja`, 중국어 `zh`, 스페인어 `es`,
  독일어 `de`, 아랍어 `ar`. → Google Cloud Translation은 **언어당 추가 비용
  없음(글자수 과금)** 이라 전부 포함. 모두 v2/v3 지원 언어.
- **Google 계정**: `appmychannel@gmail.com` — 이 계정으로 Cloud 프로젝트 +
  Translation API + 결제(무료 한도 초과 대비) 설정. API 키는 Render 환경변수로.
- **번역 동작 = 탭 번역(온디맨드)**: 메시지는 원문으로 도착, 각 수신 메시지의
  "번역" 탭 시 그때 내 선호언어로 번역(비용/지연 보수적).
- **표시 = 원문 + 번역 동시 표시**: 번역하면 원문 아래에 번역문을 함께 노출(토글 아님).

### 구현 방식 (위 결정 반영)
- 서버 `/translate`: **Google Cloud Translation v2(Basic) + API 키** 사용(서비스계정
  JSON 불필요, 가장 단순). env `GOOGLE_TRANSLATE_API_KEY`. 캐시 + 길이 상한.
- 클라: 채팅 패널에 **선호(대상) 언어 선택** 드롭다운(기본=기기 locale). 수신 버블에
  "번역" 탭 → `/translate` 호출 → 원문 아래 번역문 표시. 실패 시 원문만.
